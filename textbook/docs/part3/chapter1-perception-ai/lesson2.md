---
sidebar_position: 2
title: 3D Perception and Point Clouds
description: Understanding spatial perception through LiDAR and depth sensors
---
...
- Understand LiDAR technology and how it creates 3D maps
- Learn to process and filter point cloud data
- Implement point cloud segmentation and object detection
- Apply 3D perception to robot navigation and manipulation

## Introduction to 3D Sensing

While 2D images are rich in color and texture, they lack direct depth information. 3D sensors like LiDAR, structured light, and time-of-flight cameras provide explicit measurements of the 3D world.

### 3D Sensor Comparison

| Sensor Type | Range | Accuracy | Speed | Cost | Best For |
|-------------|-------|----------|-------|------|----------|
| LiDAR | 100+ m | 2-3 cm | 10-20 Hz | $$$ | Outdoor navigation |
| Structured Light | 0.5-5 m | 1-2 mm | 30 Hz | $$ | Indoor manipulation |
| Time-of-Flight | 0.5-10 m | 1-2 cm | 30+ Hz | $$ | General purpose |
| Stereo Vision | 0.5-50 m | Variable | 30+ Hz | $ | Passive sensing |

## Point Cloud Fundamentals

A point cloud is a set of 3D points, each with coordinates (x, y, z) and optionally color (r, g, b) or intensity.
...
Raw point clouds often contain noise that must be filtered:
...
Normals are essential for understanding surface orientation:
...
Finding planes is crucial for understanding floors, walls, and tables:
...
## 3D Object Detection with PointNet
...
:::tip Summary
- **LiDAR and depth cameras** provide direct 3D measurements of the environment
- **Point cloud processing** includes filtering, downsampling, and normal estimation
- **RANSAC plane segmentation** finds floors, walls, and tables
- **Clustering algorithms** (DBSCAN) separate individual objects
- **PointNet** enables deep learning directly on point cloud data
- **3D perception** is essential for navigation and manipulation
:::

## Practice Exercises

1. **Depth to Point Cloud**: Capture depth images and convert them to colored point clouds.

2. **Table-top Segmentation**: Segment objects on a table and compute their bounding boxes.

3. **Obstacle Detection**: Create a real-time obstacle detector using LiDAR data.

4. **3D Object Recognition**: Train PointNet on ModelNet40 dataset for object recognition.
